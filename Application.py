...
def clear_whiteboard(display):
    wb_x1, wb_x2, wb_y1, wb_y2 = whiteboard_region["x"][0], whiteboard_region["x"][1], whiteboard_region["y"][0], \
...
def setup_panel(display):
    ...
    if crop_preview is not None:
        display[preview_region_pt1[1]:preview_region_pt2[1], preview_region_pt1[0]:preview_region_pt2[0]] = cv2.resize(
            crop_preview, (crop_preview_h, crop_preview_w))
...
def whiteboard_draw(event, x, y):
    ...
    if event is cv2.EVENT_LBUTTONUP:
        ...
        color = (0, 0, 0)
        if event in [cv2.EVENT_LBUTTONDOWN, cv2.EVENT_LBUTTONUP, cv2.EVENT_RBUTTONDOWN, cv2.EVENT_RBUTTONUP,
                     cv2.EVENT_MOUSEMOVE]:
...
def predict(model, image):
    ...
    if image is not None and image.shape[0] > 0 and image.shape[1] > 0:
        ...
        best_predictions = {}  # Set empty predictions if the image is invalid
...
model = load_model("D://Pycharm//Hand Written character recognition//best_val_loss_model.h5")
